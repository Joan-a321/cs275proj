...
from self_organising_systems.coral.utils import dotdict
...
DEFAULT_DISSIPATION_RATE_PER_SPEC_DICT = {
    ...
    "AGENT_ROOT": jp.array([1.4, 1.4]),
    "AGENT_LEAF": jp.array([0.2, 0.2]),
...
DEFAULT_TYPE_COLOR_DICT = {
    ...
    "SUN": jp.array([1., 0.79, 0.82]),  # RGB: 255,255,127
    "OUT_OF_BOUNDS": jp.array([1., 0., 0.]),  # error color (should not be seen)
    "AGENT_UNSPECIALIZED": jp.array([0.65, 0.68, 0.65]),  # RGB: 166,173,166
    "AGENT_ROOT": jp.array([1., 0.65, 0.298]),  # RGB: 133,99,36
    "AGENT_LEAF": jp.array([1., 0.65, 0.298]),  # RGB: 41,125,26
    "AGENT_FLOWER": jp.array([0.94, 0.74, 0.458]),  # RGB: 255,107,181
    "CORAL_DEAD": jp.array([0.769, 0.643, 0.518]),
}
...
class DefaultTypeDef(EnvTypeDef):
  ...
  def __init__(
    ...
    self.agent_spawnable_mats = jp.array([
            types.VOID, types.AIR, types.EARTH, types.CORAL_DEAD  # 加上尸体
        ], dtype=jp.int32)
...
def get_env_and_config(
  ...
  if ec_id == "persistence":
    ...
    config = EnvConfig(
        ...
        absorbtion_amounts=jp.array([0.5, 0.5]),
        spawn_cost=jp.array([0.75, 0.75]),
        reproduce_cost=jp.array([0.1, 0.1]),
