...
from self_organising_systems.coral.environments import EnvTypeDef
from self_organising_systems.coral.environments import Environment
from self_organising_systems.coral.utils import split_2d
from self_organising_systems.coral.utils import vmap2
...
def find_fertile_soil(type_grid, etd):
  ...
  mask = (((type_grid[1:] == etd.types.EARTH) | (type_grid[1:] == etd.types.CORAL_DEAD)) &
...
def _line_gravity(env, x, h, etd):
  ...
  gravity_strength = 0.2  # 你可以把它放到 config 里
  ku, _ = jr.split(jr.PRNGKey(0))  # 或用传进来的 key
  rand_mask = jr.uniform(ku, swap_mask.shape) < gravity_strength
  swap_mask = swap_mask * rand_mask.astype(jp.float32)
  # [h] -> [1,h]
...
def process_energy(env: Environment, config: EnvConfig) -> Environment:
  ...
  #     replacement_type_grid)
  replacement_type_grid = etd.types.CORAL_DEAD * kill_agent_int + env.type_grid * (1 - kill_agent_int)
  new_type_grid = replacement_type_grid
