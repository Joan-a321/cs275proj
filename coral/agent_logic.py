...
from self_organising_systems.coral.env_logic import ExclusiveInterface
...
from self_organising_systems.coral.env_logic import ReproduceInterface
from self_organising_systems.coral.env_logic import SpawnOpData
from self_organising_systems.coral.env_logic import ParallelInterface
from self_organising_systems.coral.environments import EnvConfig
from self_organising_systems.coral.utils import stringify_class
...
class BasicAgentLogic(AgentLogic):
  ...
  def denm_init(self, key):
    ...
    keep_en = keep_en.at[:].set(
        self.config.dissipation_per_step * 10 + self.config.spawn_cost
        + self.config.specialize_cost * 3)
    # flowers keep a different amount of energy.
    keep_en = keep_en.at[self.config.etd.specialization_idxs.AGENT_FLOWER].set(
        self.config.dissipation_per_step * 10 + self.config.reproduce_cost
        + self.config.specialize_cost * 3)
    div = 1 / 10
    earth_nut_idx = 0
    air_nut_idx = 1
    ...
    #b = b.at[:, jp.array([1]), earth_nut_idx].set(div)
    # don't give air nut up

    b = b.at[:, jp.array([1]), earth_nut_idx].set(0.9)
    b = b.at[:, jp.array([3, 5]), earth_nut_idx].set(0.002)
    b = b.at[:, jp.array([6, 7, 8]), air_nut_idx].set(div)
  ...
  def excl_init(self, key):
    ...
    spawn_prob_sensitivity = jp.array([20.])
    ...
    b = b.at[spec_idxs.AGENT_UNSPECIALIZED, jp.array([3, 5, 6, 7, 8])
             ].set(div)
    b = b.at[spec_idxs.AGENT_UNSPECIALIZED, 4].set(-div * 2)  # not self
    # root spawns preferably below
    b = b.at[spec_idxs.AGENT_ROOT, jp.array([3, 5, 6, 7, 8])].set(div/100)
    b = b.at[spec_idxs.AGENT_ROOT, 4].set(-div * 2)  # not self
    # leaf spawns preferably above
    b = b.at[spec_idxs.AGENT_LEAF, jp.array([1])].set(div*2)
